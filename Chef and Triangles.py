...
def get_triangles_from_inradius(inradius, upper):
    triangles = ((i, j, k) for i in range(inradius, upper) for j in range(inradius, upper) for k in range(inradius, upper) if is_triangle(i, j, k) and get_inradius(i,j,k) == inradius)
